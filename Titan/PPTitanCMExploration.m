...
Planet.Qmantle_Wm2 = 1e12/4/pi/Planet.R_m^2; % 12 mW/m2
...
Planet.Cmeasured = 0.3318; % in prep Rambaux and Sotin 
...
% [50 75 100 125]km ice Ih

Planet.Ocean.w_ocean_pct=1; Planet.Tb_K = [268 264.9 261.5 257.6]; % 0 Wt% temperatures at the bottom of the Ice Ih
    Seismic.mantleEOS = 'CM_2percent_water_bulk_nofluid.tab';
    PlanetProfile(Planet,Seismic,Params)
% for ii = 1:length(interiors)
%     Seismic.mantleEOS = interiors{ii};
%     PlanetProfile(Planet,Seismic,Params)
% end
Planet.Ocean.w_ocean_pct=10; Planet.Tb_K =[267.1 263.6 260 255.8]; % 10 Wt% temperatures at the bottom of the Ice Ih; as currently in the manuscript
    Seismic.mantleEOS = 'CM_2percent_water_bulk_nofluid.tab';
    PlanetProfile(Planet,Seismic,Params)
% for ii = 1:length(interiors)
%     Seismic.mantleEOS = interiors{ii};
...
% Planet.Ocean.w_ocean_pct=0; Planet.Tb_K = [267]; % 0 Wt% temperatures at the bottom of the Ice Ih
% for ii = 1:length(interiors)
%     Seismic.mantleEOS = interiors{ii};
%     PlanetProfile(Planet,Seismic,Params)
% end
Planet.Ocean.w_ocean_pct=1; Planet.Tb_K = [267.1 264]; % 1 Wt% temperatures at the bottom of the Ice Ih
Planet.Ocean.w_ocean_pct=1; Planet.Tb_K = [262.5]; % 1 Wt% temperatures at the bottom of the Ice Ih
for ii = 1:length(interiors)
    Seismic.mantleEOS = interiors{ii};
    PlanetProfile(Planet,Seismic,Params)
end
