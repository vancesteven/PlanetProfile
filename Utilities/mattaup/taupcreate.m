...
if(~exist('edu.sc.seis.TauP.VelocityModel','class'))
    ...
    jars=dir('/Users/marusiak/Documents/MATLAB/seizmo/mattaup/lib/*.jar');
    %jars=dir([mypath fs 'lib' fs '*.jar']);
    for i=1:numel(jars)
        if(~ismember(strcat('/Users/marusiak/Documents/MATLAB/seizmo/mattaup/lib/',jars(i).name),javaclasspath))
            javaaddpath(strcat('/Users/marusiak/Documents/MATLAB/seizmo/mattaup/lib/',jars(i).name));
        end
    end
end
...
%     if(~any(ismember(char(fileext),'.nd' '.tvel'})))
