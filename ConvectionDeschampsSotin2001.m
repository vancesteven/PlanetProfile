...
% determine solid state convection for ices based on Deschamps and Sotin
% (2001; DS2001)
...
Dcond = [0 632 0 0 307 183]; % term for ice V fit from T^-0.6 to T^-1 from Table 1 of Andersson and Inaba

Tlith = Ttop + 0.3*(Tm-Ttop); %kludge, scales 150K from DS2001 to the generic case.
...
% Ra=alpha*rho*g*Del_Ttbl*h^3/Kappa/nu/Tm; % Sotin et al. (2010) Eqn 16
...
%   deltaTBL=(20*Kappa*nu*Tm/alpha/rho/g/Del_Ttbl)^(1/3); % thermal boundary layer -> Ra=20 - I am not sure if that is the right way to find the thermal boundary layer thickness
