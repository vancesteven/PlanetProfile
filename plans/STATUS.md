...
Updated: 2026-08-19 (Machine B: **Enceladus r7 production build ran the full 3520/3520 ocean grid (~10h), then hard-failed on r7's own C2 reachability check BEFORE saving -- diagnosed as a FALSE POSITIVE, table-precision mismatch, not a physics/build defect.** The C1 reachability table stores w_ppt rounded to 4dp; the build's actual w-axis query uses unrounded float64 -- at node (zb=6.0, w=2.8943) the two differ by 5e-6 in log-w, which through the table's steep local slope near this w shifts the interpolated zb_min floor by ~0.0002 km, just enough to flip a strict `<` comparison at a node whose achieved zb (6.0086 km) matches the table's own row EXACTLY. All 3519 other nodes passed; this is a boundary artifact at ONE node, four orders of magnitude below the table's own declared 6.3 km quantization uncertainty. Did not patch/relax the C2 check to force a pass (gates are interpreted, never tuned) -- wrote up the mechanism with three fix options (regenerate the table at full precision / round the query to match / add a tolerance band far below the table's own uncertainty) and routed for a ruling rather than guessing which fix is preferred. Full writeup: `validation_reports/enceladus_isostasy/r7_build_c2_false_positive.md`. Cost: ~10h compute produced no saved cache (`_save_cache_atomic` runs after the C2 check) -- all node computation was independently correct and does not need to be redone, only the table/query precision mismatch needs a fix before re-running. NH3 corrected-SBC PASS (below) unaffected, already complete.
Prior: 2026-08-17 (Machine B: **NH3 corrected-pipeline SBC RESULT: PASS.** 900 pairs requested -> 613 drawn (31.9% support-guard/nonfinite attrition, matches precedent) -> 485 survived the preregistered Pareto-k<=0.7/ESS>=1000 filter (20.9% excluded, under the 25% inconclusive threshold; 15 short of the 500 target -- flagged honestly, not material given every raw p-value clears 0.05 by a wide margin, min 0.171). All 13 sampled params PASS the weighted-rank KS uniformity test both raw and BH-FDR corrected. Excluded pairs skew to higher x-norm z-distance (median 1.28 vs 0.78 survived) -- same qualitative pattern as the C12 sweep, now confirmed on genuine SBC ground truths. Full result: `corrected_sbc_RESULT.md` + `corrected_sbc_report.json`. Run took ~28.2h CPU / ~14h wall at 8-way, finished overnight. Not self-adjudicated -- pushed for scientific-reviewer interpretation alongside the still-open C12 finding (this result validates the corrected pipeline where it survives its own reliability gates; it does not resolve the ~21% excluded). Enceladus r7 build still BLOCKED, no ruling yet on the zb-axis ambiguity (unchanged since 2026-08-16f, see prior entry below).)
Prior: 2026-08-16f (Machine B: **r7 build BLOCKED before any compute -- zb-axis endpoint convention in enceladus_cassini_isostasy_7D.json self-contradicts.** Ran the C1-C3 test suite (44 pass) then wrote `plans/scripts/enceladus_zbw_production_build.py` with a --dry-run axis-vs-config verification step BEFORE the real ~19.5h build call. It caught: declared `n_total: 87` only reconciles with the 5 segments' own n-values under a half-open reading ending at 44.75 km; but the config's OWN `endpoint_convention` field says segments are inclusive of both endpoints (stated explicitly to fix a prior "box edge has no node" bug), which needs 88 nodes ending at 45.0 km -- and that 45 km point is the segment's own declared "OCEAN/NO-OCEAN SUPPORT EDGE" where "the fine step must hold." The 87-node reading has no interpolant node at the exact boundary the segment exists to resolve. Did not guess and spend 19.5h of compute on a possibly-wrong axis at an E1-class edge. Full writeup: `validation_reports/enceladus_isostasy/r7_build_axis_ambiguity.md`, pushed for A's ruling. Prior: **§0.28 ruling received (N=20000, n_sbc=500, weighted-rank C14 option i, preregistered low-ESS exclusion policy) -- corrected-pipeline SBC driver built, preregistered, verified, RUNNING in background.** Wrote `corrected_sbc_PREREGISTRATION.md` fixing rank statistic (weighted empirical-CDF rank r_j = sum_k w_k*1[sample_jk<theta_0_j] per param, scipy.stats.kstest vs Uniform, BH-FDR across the 13 sampled params reusing validate_sbi.py's block), over-request arithmetic (900 pairs to net >=500 survivors after ~30% guard attrition x ~82% Pareto-k/ESS pass rate), and the INCONCLUSIVE-BY-COVERAGE threshold (excluded frac >25%) before any result exists. Driver `plans/scripts/nh3_corrected_sbc.py` reuses the C12 sweep's per-pair config+runner rebuild fix for the MCMCRunner likelihood-closure trap. --verify-only reproduced the committed fiducial Pareto-k/ESS to full precision (0.19062726587199338 / 1270.7118961718193, exact match) and the rank-formula smoke test returned 13 finite values in [0,1] before the real run started. Committed (a86f1041) before launching. Launched at --n-jobs 8 (~900 pairs x ~6.2 min/pair serial-equiv => revised estimate ~93 CPU-h, ~12 wall-h at 8-way -- the manager's 52 CPU-h figure was against 500 pairs directly, not accounting for the over-request my own preregistered arithmetic requires to net 500 SURVIVING pairs; same per-pair rate, larger n, no new ruling needed). Running in background per "run it in gaps." Enceladus: r6 NOT RATIFIED / build rescinded, unchanged since 2026-08-16d -- no new B action there.)
