...
Updated: 2026-08-16c (Machine B: **direct answer to §0.25's key question -- the C12 failing region DOES intersect the GUI-reachable box, computed from already-collected data, no new compute.** §0.25 asked whether the failing region lies beyond x_obs_limits (expected disposition: scope-bound, deploy posture unchanged). It does not. Restricted to the ONLY declared GUI limit (Im_k2<=0.30): 17/114 fail (14.9%). Restricted further to the training-support box AND Im_k2<=0.30 (narrowest reachable set): 16/113 fail (14.2%) -- most (12/17) are moderate degradations (Pareto-k 0.7-1.5) not catastrophic, but common enough that 'validated within the reachable domain' is not supportable as stated. Also flagged: Re_k2 has NO declared GUI limit at all (only Im_k2 does), so any reachable-box claim about it describes the training-support box, not an enforced deploy-time guard -- parallel to r5's D1 finding on Enceladus (unenforced ocean-branch MoI window). Full answer: validation_reports/titan_freegrav_nh3_1m/is_correction/c12_gui_box_answer.md. Not self-adjudicated -- resolves the factual question in §0.25; disposition is the reviewer's/manager's call. Enceladus: read r5 NOT RATIFIED (D1-D8 plumbing gaps, build/training/publication-blocked items triaged) and the manager's §0.26 instructions -- WATCHING for the D1 commit (ocean MoI window recorded+enforced) to START the zb-cache build immediately, without waiting for r6; dataset gen waits on D2, training waits on r6.)
