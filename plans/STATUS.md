...
Updated: 2026-08-16d (Machine B: **corrected-pipeline SBC spec pushed for A's assessment, NO compute spent per user direction to conserve tokens and only run heavy tasks as directed.** Before building, sent the underspecified §0.17 item 6 spec to an independent Opus pass -- same discipline that caught the C12 design trap. Found: the stated ~14 CPU-h budget is inconsistent with the stated method (a full n_derived=None recompute per SBC pair is structurally required; at the fiducial N=20000, 500 pairs cost ~52 CPU-h, 3.7x over; only satisfiable at N<=4400, own C12 timing data confirms the arithmetic to two digits). Corrected a wrong prior assumption of mine: NH3's joint design drops the no-ocean guard but INDEPENDENTLY sets apply_support_guard=true, so theta_0 must be drawn via generate_training_set (~30% attrition, matches the committed uncorrected SBC report), never a raw prior-box draw. Flagged: ~18% of pairs will fail the already-pushed C12 Pareto-k gate and need a preregistered low-ESS policy before any result exists; "deploy-time N" is undefined in the code (candidates 5000-50000, and the GUI's own n_derived=1000 default cannot produce IS weights at all -- separate finding worth its own fix). Full writeup + open decisions: validation_reports/titan_freegrav_nh3_1m/is_correction/corrected_sbc_SPEC_QUESTIONS.md. Enceladus: pulled r6 NOT RATIFIED (E1 zb-placement invariant fails grid-wide -- 8/8 real builds rejected by 5-27x; E2 no ocean-branch cache lookup exists) and confirmed via §0.27 that the D1-triggered cache-build authorization is RESCINDED -- did not start the build. NH3 corrected-SBC is the sole standing work item per §0.27.)
