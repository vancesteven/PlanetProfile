...
Updated: 2026-08-13 (Machine B: **ENCELADUS FROZEN-CONFIG CANDIDATE r4 + physics blockers.** Per Machine A's §0.22 Enceladus-foreground directive. `PlanetProfile/Inference/configs/enceladus_cassini_isostasy_7D.json` written as a CANDIDATE (`not implemented`, unrunnable, NOT ratified) — 5D (two dead params dropped), observables {C20,C22,C30,libration_deg}, ocean/frozen discrete branch model with frozen zb DERIVED from a declared rho_rock prior. FOUR reviewer passes; last = NEEDS-MORE-ANALYSIS. **CLOSED:** B15 (Sigma_model re-sourced from the H&M PDF Table 1, verified vs their Eq.24/25 — this RETRACTED a reviewer claim the values were ~2x too large); frozen-branch reparameterization (measured box-edge swing 22.7->43.3% and a factor-2 prior tilt toward low-density rock); mass-neutral rho_ice override; root cause of the frozen-node mass violation — **Titan CLEAN, no escalation to ratified artifacts**. **B2' REOPENED and ESCALATED — now the campaign's largest open systematic:** the libration figure convention is a STRUCTURAL COUPLING, not a scaling; four treatments give matched shell thickness 24.70/25.80/27.34 km (H&M Eq.12 gives 30.5 sigma, no match), a 2.6 km spread against the 6 km H&M-vs-Park separation the campaign exists to adjudicate. The +1.10 km is RETRACTED. Also found: the libration DOES depend on compensation_C2 (0.1005->0.1824 deg over C2 0->1), contradicting an earlier dlibration/dC2=0.00 finding measured on the hydrostatic figure. **TWO corrections to my OWN work:** my blast-radius audit used the wrong quadrature (midpoint vs PP's outer-edge) and its headline is retracted — re-run correctly, max positive residual across ~37k structures is 0.0000% and the mass gate CAN land globally; and a regression test that could not catch its own bug (synthetic stack) was replaced with a real-cached-node version. **BLOCKED on reviewer sign-off:** the TWO-PART mass fix — the guard at Silicates.py:74-92 fires then RESETS Do.VALID=True expecting a retry wired only in the Fe_CORE branch (LayerPropagators.py:1800); hoist it, THEN gate, because a gate alone makes frozen nodes unbuildable. Deep-frozen Enceladus IS feasible (zb 51.78/53.93/58.29 km at rho_rock 2314/2334/2378, all on C/MR2 0.3350), so the +/-0.08 MoI widening was never physics. Resume note: validation_reports/enceladus_isostasy/RESUME_NOTE.md. Prior entry below.)
