...
Depends on plans/scripts/v5_gen_dataset.py having produced:
  /tmp/v5_build/datasets/v5_{baseline,noinduction,nok2}_1m.npz
...
Run (after generation completes):
  mamba run -n PPcl env PYTHONPATH=. NUMBA_CACHE_DIR=/tmp/pp_numba_cache \
    KMP_DUPLICATE_LIB_OK=TRUE python plans/scripts/v5_train_all.py
"""
...
DSDIR = "/tmp/v5_build/datasets"
ARTDIR = "/tmp/v5_build/artifacts"
...
def main():
    ...
    for tag, (cfgpath, artname) in ARMS.items():
        npz = os.path.join(DSDIR, f"v5_{tag}_1m.npz")
        if not os.path.exists(npz):
            raise SystemExit(f"[train] missing dataset {npz} — run v5_gen_dataset.py first")
    ...
    with open(os.path.join(ARTDIR, "v5_train_manifest.json"), "w") as f:
        json.dump({"train_seed": TRAIN_SEED, "arms": results}, f, indent=2)
    print(f"[train] all done. manifest -> {ARTDIR}/v5_train_manifest.json")
