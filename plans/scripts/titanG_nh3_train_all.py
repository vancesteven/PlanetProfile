"""Train the Titan free-gravity NH3 JOINT nsf SBI artifact.

Depends on plans/scripts/titanG_nh3_gen_dataset.py having produced:
  /tmp/titanG_build/datasets/titanG_nh3_1m.npz

Trains one nsf flow on (theta, x={C20,C22,Re_k2,Im_k2}). NH3 train seed = 72
(per config metadata sbi_seeds_2026_08_02). Artifact lands in /tmp first, then is
...
Run (after generation completes):
  mamba run -n PPcl env PYTHONPATH=. NUMBA_CACHE_DIR=/tmp/pp_numba_cache \
    KMP_DUPLICATE_LIB_OK=TRUE python plans/scripts/titanG_nh3_train_all.py
"""
...
ARTNAME = "titan_freegrav_nh3_posterior_1m.pt"
...
def main():
    ...
    npz = os.path.join(DSDIR, "titanG_nh3_1m.npz")
    if not os.path.exists(npz):
        raise SystemExit(f"[train] missing dataset {npz} — run titanG_nh3_gen_dataset.py first")
    ...
    manifest = {
        "kind": "titanG_nh3_joint_train", "train_seed": TRAIN_SEED,
    ...
    with open(os.path.join(ARTDIR, "titanG_nh3_train_manifest.json"), "w") as f:
        json.dump(manifest, f, indent=2)
    print(f"[train] trained in {dt/60:.1f} min -> {dst}")
    print(f"[train] manifest -> {ARTDIR}/titanG_nh3_train_manifest.json")
