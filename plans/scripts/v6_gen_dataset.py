...
Outputs (all in /tmp, copied to Dropbox by the caller):
  /tmp/v6_build/datasets/v6_{baseline,noinduction,nok2}_1m.npz  (theta, x, stats)
  /tmp/v6_build/datasets/v6_gen_manifest.json  (drop rates, hashes, seeds, caveats)

Run:
  mamba run -n PPcl env PYTHONPATH=. NUMBA_CACHE_DIR=/tmp/pp_numba_cache \
    KMP_DUPLICATE_LIB_OK=TRUE python plans/scripts/v6_gen_dataset.py --n 1000000
"""
...
BASE_CFG = "PlanetProfile/Inference/configs/europa_clipper_v6_freegrav_11D.json"
ABLATIONS = {
...
OUTDIR = "/tmp/v6_build/datasets"
DATA_SEED, NOISE_SEED = 67, 6767
...
def main():
    ...
    for tag, cfgpath in ABLATIONS.items():
        ...
        out = os.path.join(OUTDIR, f"v6_{tag}_1m.npz")
    ...
    with open(os.path.join(OUTDIR, "v6_gen_manifest.json"), "w") as f:
        json.dump(manifest, f, indent=2, default=str)
    print(f"[gen] manifest -> {OUTDIR}/v6_gen_manifest.json")
