...
Updated: 2026-08-19 (Machine B: **Enceladus r7 production build completed all 3520/3520 ocean nodes (~10h) then hard-failed on r7's own C2 reachability check BEFORE `_save_cache_atomic` -- diagnosed as a FALSE POSITIVE, not a physics defect.** Root cause (zero additional compute, log inspection + table lookup): the C1 reachability table stores w_ppt rounded to 4dp, but the build's w-axis query is unrounded float64. At node (zb=6.0, w=2.8943) the true axis value 2.8942661247167516 differs from the table's rounded 2.8943 by 5e-6 in log-w; the table's local slope near this w is steep (9.25 -> 6.01 -> 12.13 km across neighboring w nodes), so that tiny precision gap shifts the interpolated zb_min floor by ~0.0002 km -- enough to flip the strict `<` check at a node whose achieved zb (6.0086 km) matches the table's OWN row value exactly. All 3519 other nodes were unaffected; this is a boundary artifact 4 orders of magnitude below the table's own declared 6.3 km quantization uncertainty. Did NOT patch or relax the C2 check to force a pass -- gates are interpreted, never tuned. Wrote up the mechanism with three fix options (regenerate the C1 table at full float64 precision / round the query to match / add a tolerance band far below the table's declared uncertainty) and routed for a ruling: `validation_reports/enceladus_isostasy/r7_build_c2_false_positive.md`. Cost: ~10h of compute produced no saved cache (the check runs before the save); every node's own computation was independently correct and does not need to be redone -- only the precision mismatch needs a fix before re-running. This is the SECOND precision/axis defect this build has surfaced (after the §0.30 endpoint ruling); recommend fixing (1) primary + (3) as defense-in-depth rather than picking one quickly. NH3 corrected-SBC (below) unaffected, already complete and PASSED.
Prior: 2026-08-17 (Machine B: **NH3 corrected-pipeline SBC (§0.28) RESULT: PASS.** 900 requested -> 613 drawn (31.9% attrition, matches precedent) -> 485 survived the preregistered Pareto-k<=0.7/ESS>=1000 filter (20.9% excluded, under the 25% inconclusive threshold; 15 short of the 500 target, flagged honestly -- not material, min raw p-value 0.171 clears 0.05 by a wide margin). All 13 params PASS raw and BH-FDR corrected. Excluded pairs skew to higher x_norm z-distance (median 1.28 vs 0.78 survived), same pattern as C12, now on genuine SBC pairs. `corrected_sbc_RESULT.md` + `corrected_sbc_report.json`. Ran ~28.2h CPU / ~14h wall, finished overnight, no crashes across the full 613-pair sweep. Not self-adjudicated -- routed to the scientific-reviewer alongside the still-open C12 finding (this validates the corrected pipeline where it clears its own reliability gates; does not resolve the ~21% it excludes). Enceladus r7 build still BLOCKED pending A's ruling on the zb-axis ambiguity below, unchanged.)
Prior: 2026-08-16f (Machine B: **r7 build BLOCKED before any compute -- zb-axis endpoint convention in enceladus_cassini_isostasy_7D.json is self-contradictory.** Wrote plans/scripts/enceladus_zbw_production_build.py (--dry-run verifies the constructed axes against the config's declared structure_cache_spec before calling build_zbw_grid_cache; ran the 44 C1-C3 tests first, all pass). --dry-run caught: the declared zb_km_grid.n_total=87 only reconciles with the 5 segments' own `n` values under a half-open-per-segment reading that ends the axis at 44.75 km -- but the SAME config's endpoint_convention field says segments are "inclusive of BOTH endpoints" (explicitly to fix a prior bug where "the box edge [had] no node"), which requires 88 nodes ending at 45.0 km. The 42-45 km segment's own rationale calls that exact 45 km point the "OCEAN/NO-OCEAN SUPPORT EDGE... the fine step must hold across the w axis" -- so the 87-node reading has no interpolant node at the one boundary the segment exists to resolve. Full writeup: validation_reports/enceladus_isostasy/r7_build_axis_ambiguity.md. Did not guess and build (19.5h of compute on a possibly-wrong axis, at the exact edge E1-style bugs live). NH3 corrected-SBC continues unaffected in the background per §0.28.)
Prior: 2026-08-16e (Machine B: §0.28 ruling received and executed -- corrected-pipeline SBC preregistered (`corrected_sbc_PREREGISTRATION.md`: weighted-rank r_j = sum_k w_k*1[sample_jk<theta_0_j] per param, scipy.stats.kstest vs Uniform + BH-FDR across 13 params reusing validate_sbi.py's block; 900-pair over-request to net >=500 survivors past ~30% guard attrition x ~82% Pareto-k/ESS pass; INCONCLUSIVE-BY-COVERAGE at >25% excluded), driver built (`plans/scripts/nh3_corrected_sbc.py`, reuses the C12 per-pair rebuild fix for the likelihood-closure trap), --verify-only reproduced the committed fiducial Pareto-k/ESS to full precision + rank-formula smoke test passed, committed (a86f1041), launched at --n-jobs 8 in background. Revised cost note: 900 pairs (not 500) at ~6.2 min/pair serial-equiv is ~93 CPU-h / ~12 wall-h at 8-way -- same per-pair rate as the manager's 52 CPU-h/500-pair figure, just applied to the over-request size my own preregistered arithmetic requires; not a new ruling, flagging for visibility. Report lands in `validation_reports/titan_freegrav_nh3_1m/is_correction/corrected_sbc_report.json` when done.)
Prior: 2026-08-16 (Machine B: C12 amortized sweep on deployed NH3 artifact FAILS (29/149, 19.5%, threshold <=5%) -- no prior driver existed, built plans/scripts/nh3_c12_amortized_sweep.py with a preregistration doc (c12_sweep_PREREGISTRATION.md) fixing every design choice before running. Design trap caught before compute: the MCMCRunner likelihood closure is bound to config.observables at construction and ignores a swept x_obs argument -- fixed by rebuilding config+runner per point. Verified against the committed fiducial (Pareto-k/ESS match to full precision) before the real sweep. Result: failures concentrate at high z-distance in x_norm space (mean z=3.48 failed vs 1.59 passed), driven by Re_k2/Im_k2 tails -- overlaps the existing k2-quarantine discussion rather than implicating C20/C22. Endpoint failures: C22-hi hard C4 rejects (75.6% sentinel), Re_k2-lo and Im_k2-hi marginally fail Pareto-k; the other 5 endpoints pass cleanly. ESS/N and 0.9326-sanity-line deliverables also reported per spec (median ESS/N 0.287 across the sweep vs 0.0635 at the fiducial; ocean-branch flow-vs-corrected swings up to 12.7pp off-fiducial vs 0.33pp at the fiducial). Full findings + reviewer questions in c12_sweep_FINDINGS.md. Does not touch the already-passed fiducial gates. No production code changed. Enceladus: A2-A7 confirmed landed + manager-verified on catch-up; next Machine B step stays gated on Machine A's r5 ratification pass.)
