...
Updated: 2026-08-14 (Machine B: B2' ADJUDICATED — scientific-reviewer verdict BLOCK; remains BLOCKING but the block lands on the SHIPPED treatment, not the measurement. The shipped `H22_obs_m` path (the config's adopted "option A") is NOT a valid treatment: the shell-base interface's physical weight in Ks is Delta_rho ~80-95 kg/m3 but appears as a difference of two ~10x larger cancelling terms, so scaling `(Bs-As)` applies an implicit sign-flipping effective scale of +0.33 to -0.58 to the base figure while the docstring claims interior interfaces stay hydrostatic. Identity `Ks/(3 omega^2) = rho_ice*f_top + Delta_rho*f_base` verified ~1e-14. Defensible: hydrostatic and surface-only; Delta_rho-consistent Eq.-12 bracketed by C2 in 25.99-27.34 km. TWO prior headline numbers are ARTIFACTS of the same mis-weighting: Eq.-12 "30.5 sigma / no match" -> +1.66 sigma at zb 25.99, and the C2 sweep "0.1005->0.1824 deg" -> 1.6 sigma span (~20x smaller), so MAJOR-4's channel split largely SURVIVES. HEADLINE RATIFIED: the H&M-vs-Park separation is essentially entirely the libration measurement revision, Delta=+5.6 km at H&M's own published core parameters (19.5-19.7 -> 25.1-25.4 km, inside their JOINT Table 2 band and Park's respectively) vs a ~5.5 km band-centre separation; the two bands are NOT competing answers and the deliverable needs restating. Frozen-branch ruling unaffected. PP's 2x2 solve is algebraically identical to H&M Eq. 20 (rel <=2.2e-16); coefficients still uncertified. The published-answer test did NOT discriminate. Third self-correction recorded: my "whole-difference and surface-only are the same code path" claim was wrong. USER DECISION NEEDED on retiring/repairing the whole-difference scaling (it moves a test pinning the suspect +1.40-1.43 sigma shift, and the recorded option-A user decision rested on falsified premises). Reviewer recommends a standing Delta_rho invariant on the libration module. Test baseline 11 passed + 1 xfail. No production code changed. Full state: validation_reports/enceladus_isostasy/RESUME_NOTE.md + b2prime_ADJUDICATED_drho_weighting.json. Prior entry below.)
