...
### C21 — Enceladus doc/test odds and ends (freeze non-blockers N2/N5 + supersessions) [status: verified]
...
**Report (Codex, 2026-08-14):** `verified`. The constraints spec now uses
Park et al. 2024 + erratum values (C20/C22, corr=0.47, R_ref=256.6 km,
libration 0.092±0.003 deg), records the implemented/wired libration channel,
and rejects hypothetical k2. STRATEGY exception (b) is closed by C17. Freeze
metadata records the adjudicated rigid=True handling: +0.8% correction,
±0.4% residual, rheology-change invalidation, and the coupled post-campaign
K_int repair; the falsified 0.03% claim is explicitly struck. Added the N2
GSW regression across 42/70/100 ppt at 3 MPa and 271–274 K. The Titan methods
HTML regenerated byte-identically from Markdown with its generator stamp.
Verification: the new plus existing Enceladus conductivity tests pass 6/6;
four Markdown sources render; local references and generated stamp pass;
`py_compile` and `git diff --check` pass. Implementation `a6a55631`.
