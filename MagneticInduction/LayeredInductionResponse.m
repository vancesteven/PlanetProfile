function outWaveforms = LayeredInductionResponse(Planet,FTdata,Params)
...
elseif isfield(Planet,'ionos_bounds')
...
% consider a case with an ocean under high pressure ice
...
% compare nominal calculations with equivalent uniformly conducting oceans
if isfield(Planet,'COMPARE_UNIFORM_CONDUCTIVITY_OCEANS') && Planet.COMPARE_UNIFORM_CONDUCTIVITY_OCEANS
...
if Planet.PLOT_SIGS    
    ...
    l_str = {'main field'};
    if isfield(Planet,'ionos_bounds')
        plot(sPedersen_mionos,b_mionos/1e3,...
        sPedersen_ionos,b_ionos/1e3);
        l_str = {l_str{:},'with ionosphere','ionosphere only'};
    end
    if isfield(Planet,'COMPARE_UNIFORM_CONDUCTIVITY_OCEANS') && Planet.COMPARE_UNIFORM_CONDUCTIVITY_OCEANS
        plot(s_mean,b_mean/1e3,...
        s_top,b_top/1e3);
        l_str = {l_str{:},'mean ocean','top ocean'};
    end    
    if isfield(Planet,'sub_ocean')
        plot(sPedersen_subo,b_subo/1e3,'--',...
            sPedersen_subo_ionos,b_subo_ionos,'--');
        l_str = {l_str{:},'sub-HPice ocean','sub-HPice with ionosphere'};
    end
    legend(l_str,'NumColumns',2,'Location','northwest');
%     legend('boxoff')
    xlabel(lbl.sigma)
    ylabel(lbl.r_kms)
    set(gca,'xscale','log')
