...
contfig = 111;
for ibody = 1:nbodies
    if ~ishandle(contfig+ibody); figure(contfig+ibody); end
end
...
for ibody = 1:nbodies
    ...
    if CALC_NEW
        ...
        DO_LEGEND = cfg.do_legend;
        PLOT_V2020s = cfg.plot_V2020s; 
        DO_EUROPA = cfg.do_europa;
        DO_GANYMEDE = cfg.do_ganymede;
        DO_CALLISTO = cfg.do_callisto;
    end

    intMethod = 'makina';

    set(0, 'CurrentFigure', contfig+ibody);
    set(gcf, 'Name', ['Induced B contour plot ' lname])
    clf
    ...
    fprintf("Printed contour plot: %d\n", contfig+ibody)
    
    str2020 = '';
    if PLOT_V2020s
        str2020 = '_WithV2020Models';
    end
    print(contfig+ibody,cfg.fig_fmt,fullfile([lname '/figures/' 'MagPhase' lname str2020 '.eps']));
end
end
