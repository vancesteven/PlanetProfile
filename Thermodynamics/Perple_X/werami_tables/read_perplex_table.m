...
den=reshape(den,tstep,pstep);vp=reshape(vp,tstep,pstep);vs=reshape(vs,tstep,pstep);cp=reshape(cp,tstep,pstep);alpha=reshape(alpha,tstep,pstep);
...
out.alpha=alpha;
if exist('Ks')
    Ks=reshape(Ks,tstep,pstep);Gs=reshape(Gs,tstep,pstep);
