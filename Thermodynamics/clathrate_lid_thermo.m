function [Q_converge, T_clath_ice , deltaTBL_m,eTBL_m,Tc,rhoIce,alphaIce,CpIce,kIce,nu,CONVECTION_FLAG_I]=clathrate_lid_thermo(Tsurf, Tbasal, P, n_clath,n_ice, h_ice, clath_depth, g)
...
for int=(floor(Tsurf-floor(Tsurf))+1:(Tbasal)-floor(Tsurf))
   ...
   % Q_Wm2_clath(int)=0.5*(T_clath_ice_check(int)-Tsurf)/clath_depth;
    [Q_Wm2_clath(int),deltaTBL_m,eTBL_m,Tc,rhoIce,alphaIce,CpIce,kIce,nu,CONVECTION_FLAG_I]=...
        ConvectionDeschampsSotin2001(Tsurf,T_clath_ice_check(int),P1,clath_depth,g,33);
    [Q_Wm2_ice(int),deltaTBL_m,eTBL_m,Tc,rhoIce,alphaIce,CpIce,kIce,nu,CONVECTION_FLAG_I]=...
...
%         
    ...
    if Ra>Ra_crit % Convection % this may be a kluge; check nu and Kappa, and read the literature to confirm that 10^5 is considered sufficient as indicated by DS2001Fig3
        ...
        if eTBL_m > clath_depth
            ci_ratio=clath_depth/(eTBL_m);
             k=kIce+ci_ratio*(kclath-kIce);
