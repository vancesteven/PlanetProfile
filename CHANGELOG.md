...
### Enhancements
- **Added 3D lateral structure module with optional healpy dependency.** PlanetProfile now supports spatially-varying (3D) interior structure calculations via the new `PlanetProfile.Lateral` module. Geographic grids can be initialized with HEALPix (equal-area, requires healpy) or lat-lon (simple, no dependencies). Spherical harmonic transforms use 4π-normalization consistent with MoonMag. Enables calculation of H(r,θ,φ) for reproducing geographic tidal heating distributions (Tobie et al. 2005 Figure 10) and asymmetric magnetic induction from spatially-varying ice-ocean boundary topography. **Lateral plotting module** provides lat/lon surface maps for ice thickness, tidal heating, basal temperature, ocean conductivity, clathrate fraction, effective thermal conductivity, multi-panel summaries, by-layer heating (following Tobie et al. 2005 Fig. 10), and depth profiles. Maps use pcolormesh with optional contours, and handle missing data gracefully. **Main.py integration** seamlessly runs 3D lateral structure calculations when `Planet.Lateral.DO_3D = True` via automatic invocation of `RunLateral3D()` after gravity calculations. Backward compatible: 1D workflow unchanged when `DO_3D = False` (default). 3D workflow: 1D reference → lateral columns → mass conservation → I/O → plotting. **Documentation:** All `Planet.Lateral.*` fields documented in `VARIABLE_REFERENCES.md` with units, descriptions, and references to Górski et al. (2005) HEALPix, Styczinski et al. (2021) 4π SH normalization, Tobie et al. (2005) geographic tidal dissipation, and Ojakangas & Stevenson (1989) tidal forcing. **Optional dependency:** healpy>=1.16.0 (install with `pip install PlanetProfile[lateral]` or `conda install -c conda-forge healpy`). Graceful fallback to lat-lon grids if healpy unavailable. New classes: `LateralSubstruct` in `defineStructs.py`, modules: `SpatialGrid.py`, `TidalHeating3D.py`, `LateralStructure.py`, `MassConservation.py`, `ClathrateLateral.py`, `LateralIO.py`, `LateralPlots.py`, Main.py integration, tests: `PPTestLateralPhase1.py` through `PPTestLateralPhase9.py`. New config flag: `Params.PLOT_LATERAL` (default True) to enable/disable lateral plotting. Literature: Ojakangas & Stevenson (1989), Tobie et al. (2005), Górski et al. (2005), Hobbs (1974), Styczinski et al. (2021). (Phase 1-10 of lateral heating port: +4150 lines, 59 tests)
