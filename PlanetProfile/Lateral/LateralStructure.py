...
def RunLateralColumns(Planet, Params):
    ...
    # Compute Tb_K for each column from desired ice thickness.
    # Use the reference model's P(z) profile to get the pressure at the
    # desired ice-ocean boundary depth, then use the freezing curve to
    # get Tb_K at that pressure.
    nSurf = Planet.Steps.nSurfIce
    z_ref_m = Planet.z_m[:nSurf+1]
    P_ref_MPa = Planet.P_MPa[:nSurf+1]

    # Build a wide-range meltEOS for computing freezing temperatures across
    # all column pressures. The standard meltEOS has a very narrow T range
    # (±0.1 K around Tb_K) which causes fn_phase clamping issues at pressures
    # where Tfreeze differs from Tb_K.
    ...
    for i in range(nPix):
        ...
        # If clathrate fraction varies laterally
        ...
        # If tidal heating varies laterally
    ...
    # Set progress tracking for HydroOnly's profile completion indicator
    import time
    Params.nModels = nPix
    Params.tStart_s = time.time()

    # Run columns (parallel or serial)
...
def _RunColumnsSerial(columnPlanets, Params):
    ...
    for i, colPlanet in enumerate(columnPlanets):
        # Skip columns that already failed (e.g. during Tb_K computation)
...
def _ExtractColumnSummaries(Lateral, columnPlanets):
    ...
    for i, colPlanet in enumerate(columnPlanets):
        ...
        # Basal ice temperature (at ice-ocean interface)
        ...
        # Mean ocean conductivity — compute directly from ocean EOS
        # (ElecConduct requires inner layers that HydroOnly doesn't provide)
...
def UpdateAsymShapeFrom3D(Planet, columnPlanets):
    ...
    # Compute ice-ocean boundary radius at each grid point
    ...
    # Mean ice-ocean boundary radius
    r_mean = np.nanmean(r_iceOcean)

    # Deviations from mean (for SH decomposition)
    dr = r_iceOcean - r_mean

    # Replace NaN with zero deviation
    dr[np.isnan(dr)] = 0.0

    # Decompose deviations into SH coefficients
    ...
    # Store in format compatible with SetAsymShape
    ...
    for p in range(1, pMax + 1):
        for q in range(p + 1):
            ...
            # Apply to ice-ocean boundary (layer 0 = outermost conducting boundary)
            # Scale concentrically to other boundaries (same as CONCENTRIC_ASYM)
...
def RunLateral3D(Planet, Params):
    ...
    # Step 1: Initialize grid and ice thickness
    Planet = InitLateralStructure(Planet, Params)

    # Step 2: Lateral clathrate variation
    if Planet.Lateral.DO_CLATH_LATERAL:
        ...
        # Compute self-consistent ice thickness from clathrate-modified conductivity
        Planet.Lateral.dIce_m = EstimateIceThicknessFromClathrate(Planet)

    # Step 3: Run lateral columns
    columnPlanets = RunLateralColumns(Planet, Params)

    # Step 4: 3D tidal heating with feedback
    ...
    # Step 6: Update asymmetric shape for MoonMag
    ...
    # Step 7: Run asymmetric induction (if configured)
    if Params.CALC_NEW_INDUCT and hasattr(Planet.Magnetic, 'CALC_ASYM') and Planet.Magnetic.CALC_ASYM:
    ...
    # Step 8: Multi-degree Love numbers (if configured)
    ...
    # Step 9: Save 3D fields
    from PlanetProfile.Lateral.LateralIO import SaveLateralResults
    SaveLateralResults(Planet, Params)

    # Step 10: Generate lateral plots
    if hasattr(Params, 'PLOT_LATERAL') and Params.PLOT_LATERAL:
