...
class BulkSubstruct():
    ...
    def __init__(self):
        ...
        self.zbTol_km = None  # (Optional - used with Do.ICEIh_THICKNESS) Shell-thickness placement tolerance in km REQUIRED by the caller of GetIceShellTFreeze. None (the default, and therefore every template) keeps the historical behaviour, in which the Tb root-find's tolerance is the SHARED phase-diagram step Planet.TfreezeRes_K = 0.05 K. Set it when the achieved zb must land on a target to a stated accuracy: the root-find's temperature tolerance is then DERIVED from this km tolerance and the body's own measured |d(zb)/d(Tb)| (LayerPropagators._zbDrivenXtol), which is the only way to make placement accuracy a property of the request rather than of the phase-diagram sampling step.
