...
from PlanetProfile.Thermodynamics.HydroEOS import GetOceanEOS, GetPlanetOceanEOS, GetPlanetIceEOS, GetOceanEOSLabel, GetIceEOSLabel
from PlanetProfile.Thermodynamics.InnerEOS import GetPlanetInnerEOS
...
def SetupInit(Planet, Params):
    ...
    # Core settings
    if Planet.Do.Fe_CORE:
        # (Re)set a predefined core radius, i.e. from Planet.Do.ConstantProps['Inner'], so that we can
    ...
    if not Planet.Do.NO_OCEAN or Planet.Do.NO_OCEAN_EXCEPT_INNER_ICES:
        ...
        if Planet.Do.ICEIh_THICKNESS:
            ...
            if Planet.Do.BOTTOM_ICEV:
                ...
                TmeltI_K = np.arange(Planet.Bulk.Tb_K - Planet.TfreezeRes_K*2, Planet.Bulk.Tb_K + Planet.TfreezeRes_K*2, Planet.TfreezeRes_K)
                TmeltIII_K = np.arange(Planet.Bulk.TbIII_K - Planet.TfreezeRes_K*2, Planet.Bulk.TbIII_K + Planet.TfreezeRes_K*2, Planet.TfreezeRes_K)
                TmeltV_K = np.arange(Planet.Bulk.TbV_K - Planet.TfreezeRes_K*2, Planet.Bulk.TbV_K + Planet.TfreezeRes_K*2, Planet.TfreezeRes_K)
                ...
                Tmelt_K = np.arange(Planet.Bulk.Tb_K - Planet.TfreezeRes_K*2, Planet.Bulk.Tb_K + Planet.TfreezeRes_K*2, Planet.TfreezeRes_K)
        ...
        if Planet.Do.POROUS_ROCK:
            ...
            if Planet.Do.NO_DIFFERENTIATION or Planet.Do.PARTIAL_DIFFERENTIATION:
                ...
                for icePhase in ['Ih', 'II', 'III', 'V', 'VI']:
                    if not Params.PRELOAD_EOS_IN_PROGRESS:
                        Planet.Ocean.surfIceEOS[icePhase] = GetPlanetIceEOS(Planet, Params, Ppore_MPa, Tpore_K, icePhase)
...
def ValidateConstantProps(Planet):
    """Validate constant-property settings are properly set.
    ...
    if Planet.Do.ConstantProps['Ice']:
        for phase, iceConstantProps in Planet.Ocean.IceConstantProps.items():
            phaseId = PhaseInv(phase)
            if iceConstantProps.rho_kgm3 is None:
                log.warning(f"Do.ConstantProps['Ice'] is True but Ocean.IceConstantProps[{phase}].rho_kgm3 is not set. "
                    "Assign a density value in the PPBody.py file.")
            if iceConstantProps.Cp_JkgK is None:
                iceConstantProps.Cp_JkgK = Constants.Cp_JkgK[phase]
                log.warning(f"Ocean.IceConstantProps[{phase}].Cp_JkgK not set; defaulting to heat capacity of ice, {Constants.Cp_JkgK[phase]} J/kg/K.")
            if iceConstantProps.alpha_pK is None:
                iceConstantProps.alpha_pK = Constants.alphaIce_pK[phase]
                log.warning(f"Ocean.IceConstantProps[{phase}].alpha_pK not set; defaulting to thermal expansivity of ice, {Constants.alphaIce_pK[phase]} 1/K.")
            if iceConstantProps.kTherm_WmK is None:
                iceConstantProps.kTherm_WmK = Constants.kThermIce_WmK[phase]
                log.warning(f"Ocean.IceConstantProps[{phase}].kTherm_WmK not set; defaulting to thermal conductivity of ice, {Constants.kThermIce_WmK[phase]} W/m/K.")
            if iceConstantProps.VP_kms is None:
                iceConstantProps.VP_kms = np.nan
            if iceConstantProps.VS_kms is None:
                iceConstantProps.VS_kms = np.nan
            if iceConstantProps.KS_GPa is None:
                iceConstantProps.KS_GPa = Constants.KS_GPa[phaseId]
                log.warning(f"Ocean.IceConstantProps[{phase}].KS_GPa not set; defaulting to bulk modulus of ice, {Constants.KS_GPa[phaseId]} GPa.")
            if iceConstantProps.GS_GPa is None:
                iceConstantProps.GS_GPa = Constants.GS_GPa[phaseId]
                log.warning(f"Ocean.IceConstantProps[{phase}].GS_GPa not set; defaulting to shear modulus of ice, {Constants.GS_GPa[phaseId]} GPa.")
            if iceConstantProps.sigma_Sm is None:
                iceConstantProps.sigma_Sm = Planet.Ocean.sigmaIce_Sm[phase]
                log.warning(f"Ocean.IceConstantProps[{phase}].sigma_Sm not set; defaulting to electrical conductivity of ice, {Planet.Ocean.sigmaIce_Sm[phase]} S/m.")
            if iceConstantProps.eta_Pas is None:
                iceConstantProps.eta_Pas = Constants.etaIce_Pas
                log.warning(f"Ocean.IceConstantProps[{phase}].eta_Pas not set; defaulting to dynamic viscosity of ice, {Constants.etaIce_Pas} Pa·s.")
            if iceConstantProps.TviscTrans_K is None:
                iceConstantProps.TviscTrans_K = Constants.TviscIce_K
                log.warning(f"Ocean.IceConstantProps[{phase}].TviscTrans_K not set; defaulting to viscosity transition temperature of ice, {Constants.TviscIce_K} K.")
    
    if Planet.Do.ConstantProps['Ocean']:
        constantProps = Planet.Ocean.ConstantProps
        if constantProps.rho_kgm3 is None:
            raise ValueError(
                "Do.ConstantProps['Ocean'] is True but Ocean.ConstantProps.rho_kgm3 is not set. "
                "Assign a density value in the PPBody.py file.")
        if constantProps.Cp_JkgK is None:
            constantProps.Cp_JkgK = Constants.CpWater_JkgK
            log.warning("Ocean.ConstantProps.Cp_JkgK not set; defaulting to heat capacity of water, {Constants.CpWater_JkgK} J/kg/K.")
        if constantProps.alpha_pK is None:
            constantProps.alpha_pK = Constants.alphaWater_pK
            log.warning("Ocean.ConstantProps.alpha_pK not set; defaulting to thermal expansivity of water, {Constants.alphaWater_pK} 1/K.")
        if constantProps.kTherm_WmK is None:
            constantProps.kTherm_WmK = Constants.kThermWater_WmK
            log.warning("Ocean.ConstantProps.kTherm_WmK not set; defaulting to thermal conductivity of water, {Constants.kThermWater_WmK} W/m/K.")
        if constantProps.eta_Pas is None:
            constantProps.eta_Pas = Constants.etaH2O_Pas
            log.warning("Ocean.ConstantProps.eta_Pas not set; defaulting to dynamic viscosity of water, {Constants.etaWater_Pas} Pa·s.")
        if constantProps.TviscTrans_K is None:
            constantProps.TviscTrans_K = np.nan
            log.warning("Ocean.ConstantProps.TviscTrans_K not set; defaulting to viscosity transition temperature of water, {Constants.TviscWater_K} K.")
        if constantProps.VP_kms is None:
            constantProps.VP_kms = Constants.VPOcean_kms
            log.warning("Ocean.ConstantProps.VP_kms not set; defaulting to P-wave velocity of ocean, {Constants.VPOcean_kms} km/s.")
        if constantProps.VS_kms is None:
            constantProps.VS_kms = Constants.VSOcean_kms
            log.warning("Ocean.ConstantProps.VS_kms not set; defaulting to S-wave velocity of ocean, {Constants.VSOcean_kms} km/s.")
        if constantProps.KS_GPa is None:
            constantProps.KS_GPa = np.nan
            log.warning("Ocean.ConstantProps.KS_GPa not set; defaulting to bulk modulus of ocean, {Constants.KS_GPa} GPa.")
        if constantProps.GS_GPa is None:
            constantProps.GS_GPa = np.nan
            log.warning("Ocean.ConstantProps.GS_GPa not set; defaulting to shear modulus of ocean, {Constants.GS_GPa} GPa.")
        ...
        if Planet.Bulk.Tb_K is None:
            ...
            if not Planet.Do.ConstantProps['Ice']:
                msg += "Since Do.ConstantProps['Ice'] is False, temperature at ice-ocean interface is required to propogate ice properties from Seafreeze."
    ...
    if Planet.Do.ConstantProps['Inner']:
        ...
        constantSilProps.rho_kgm3 = Planet.Sil.rhoSilWithCore_kgm3 #TODO Change this
        if constantSilProps.Cp_JkgK is None:
            constantSilProps.Cp_JkgK = np.nan
            log.warning("Sil.ConstantProps.Cp_JkgK not set; defaulting to heat capacity of silicate, {Constants.CpSil_JkgK} J/kg/K.")
        if constantSilProps.alpha_pK is None:
            constantSilProps.alpha_pK = np.nan
            log.warning("Sil.ConstantProps.alpha_pK not set; defaulting to thermal expansivity of silicate, {Constants.alphaSil_pK} 1/K.")
        if constantSilProps.kTherm_WmK is None:
            constantSilProps.kTherm_WmK = Constants.kThermSil_WmK
            log.warning(f"Sil.ConstantProps.kTherm_WmK not set; defaulting to {Constants.kThermSil_WmK} W/m/K.")
        if constantSilProps.eta_Pas is None:
            constantSilProps.eta_Pas = Constants.etaRock_Pas
            log.warning("Sil.ConstantProps.eta_Pas not set; defaulting to dynamic viscosity of water, {Constants.etaWater_Pas} Pa·s.")
        if constantSilProps.TviscTrans_K is None:
            constantSilProps.TviscTrans_K = Constants.TviscRock_K
            log.warning("Sil.ConstantProps.TviscTrans_K not set; defaulting to viscosity transition temperature of water, {Constants.TviscTransWater_K} K.")
        if constantSilProps.VP_kms is None:
            constantSilProps.VP_kms = np.nan
            log.warning("Sil.ConstantProps.VP_kms not set; defaulting to P-wave velocity of water, {Constants.VPWater_kms} km/s.")
        if constantSilProps.VS_kms is None:
            constantSilProps.VS_kms = np.nan
            log.warning("Sil.ConstantProps.VS_kms not set; defaulting to S-wave velocity of water, {Constants.VSWater_kms} km/s.")
        if constantSilProps.KS_GPa is None:
            constantSilProps.KS_GPa = np.nan
            log.warning("Sil.ConstantProps.KS_GPa not set; defaulting to bulk modulus of water, {Constants.KSWater_GPa} GPa.")
        if constantSilProps.GS_GPa is None:
            constantSilProps.GS_GPa = Constants.GS_GPa[Constants.phaseSil]
            log.warning("Sil.ConstantProps.GS_GPa not set; defaulting to shear modulus of water, {Constants.GSWater_GPa} GPa.")
        if constantSilProps.sigma_Sm is None:
            constantSilProps.sigma_Sm = np.nan
            log.warning("Sil.ConstantProps.sigma_Sm not set; defaulting to electrical conductivity of water, {Constants.sigmaWater_Sm} S/m.")

        # Check core properties are properly set
        constantCoreProps = Planet.Core.ConstantProps
        constantCoreProps.rho_kgm3 = Planet.Core.rhoFeS_kgm3 * Planet.Core.rhoFe_kgm3 \
            * (Planet.Core.xFeS * Constants.m_gmol['FeS'] + (1 - Planet.Core.xFeS) * Constants.m_gmol['Fe'] ) \
            / (Planet.Core.xFeS * Constants.m_gmol['FeS'] * Planet.Core.rhoFe_kgm3 + (1 - Planet.Core.xFeS) * Constants.m_gmol['Fe'] * Planet.Core.rhoFeS_kgm3) #TODO Change this
        if constantCoreProps.Cp_JkgK is None:
            constantCoreProps.Cp_JkgK = np.nan
            log.warning("Core.ConstantProps.Cp_JkgK not set; defaulting to heat capacity of core, {Constants.CpCore_JkgK} J/kg/K.")
        if constantCoreProps.alpha_pK is None:
            constantCoreProps.alpha_pK = np.nan
            log.warning("Core.ConstantProps.alpha_pK not set; defaulting to thermal expansivity of core, {Constants.alphaCore_pK} 1/K.")
        if constantCoreProps.kTherm_WmK is None:
            constantCoreProps.kTherm_WmK = Constants.kThermFe_WmK
            log.warning("Core.ConstantProps.kTherm_WmK not set; defaulting to {Constants.kThermCore_WmK} W/m/K.")
        if constantCoreProps.eta_Pas is None:
            constantCoreProps.eta_Pas = Constants.etaFeSolid_Pas
            log.warning("Core.ConstantProps.eta_Pas not set; defaulting to dynamic viscosity of core, {Constants.etaCore_Pas} Pa·s.")
        if constantCoreProps.TviscTrans_K is None:
            constantCoreProps.TviscTrans_K = Constants.TviscFe_K
            log.warning("Core.ConstantProps.TviscTrans_K not set; defaulting to viscosity transition temperature of core, {Constants.TviscCore_K} K.")
        if constantCoreProps.VP_kms is None:
            constantCoreProps.VP_kms = np.nan
            log.warning("Core.ConstantProps.VP_kms not set; defaulting to P-wave velocity of core, {Constants.VPCore_kms} km/s.")
        if constantCoreProps.VS_kms is None:
            constantCoreProps.VS_kms = np.nan
            log.warning("Core.ConstantProps.VS_kms not set; defaulting to S-wave velocity of core, {Constants.VSCore_kms} km/s.")
        if constantCoreProps.KS_GPa is None:
            constantCoreProps.KS_GPa = np.nan
            log.warning("Core.ConstantProps.KS_GPa not set; defaulting to bulk modulus of core, {Constants.KS_GPa} GPa.")
        if constantCoreProps.GS_GPa is None:
            constantCoreProps.GS_GPa = Constants.GS_GPa[Constants.phaseFe]
            log.warning("Core.ConstantProps.GS_GPa not set; defaulting to shear modulus of core, {Constants.GS_GPa} GPa.")
        if constantCoreProps.sigma_Sm is None:
            constantCoreProps.sigma_Sm = np.nan
            log.warning("Core.ConstantProps.sigma_Sm not set; defaulting to electrical conductivity of core, {Constants.sigmaCore_Sm} S/m.")

def PrecomputeEOS(PlanetList, Params):
    """ Pre-generate all EOS objects needed for parallel processing of the grid.
        This avoids the overhead of recreating EOS objects in each worker process.
        
        Args:
            PlanetGrid (ndarray): Grid of Planet objects to be processed
            Params (ParamsStruct): Parameters for the run
            
        Returns:
            None (EOS objects are stored in global EOSlist.loaded)
    """
    log.info('Pre-generating EOS objects for parallel processing...')
    Params = deepcopy(Params)
    PlanetList = deepcopy(PlanetList)
    Params.PRELOAD_EOS_IN_PROGRESS = True
    # Collect all unique EOS configurations needed across the grid
    oceanPlanets = []
    ...
    minPmelt_MPa = Planet.PfreezeLower_MPa
    minTmelt_K = Planet.TfreezeLower_K
    deltaPmelt = Planet.PfreezeRes_MPa
    ...
    for i, Planet in enumerate(PlanetList.flatten()):
        ...
        if not Planet.Do.NO_H2O:
            if not Planet.Do.NO_OCEAN:
                oceanPlanets.append(Planet)
                minPmelt_MPa = min(minPmelt_MPa, Planet.PfreezeLower_MPa)
                maxPmelt_MPa = max(maxPmelt_MPa, Planet.PfreezeUpper_MPa)
                minTmelt_K = min(minTmelt_K, Planet.TfreezeLower_K)
                maxTmelt_K = max(maxTmelt_K, Planet.TfreezeUpper_K)
                deltaPmelt = min(deltaPmelt, Planet.PfreezeRes_MPa)
                deltaTmelt = min(deltaTmelt, Planet.TfreezeRes_K)
                
                minPOcean_MPa = min(minPOcean_MPa, Planet.PfreezeLower_MPa)
                maxPOcean_MPa = max(maxPOcean_MPa, Planet.Ocean.PHydroMax_MPa)
                maxTOcean_K = max(maxTOcean_K, Planet.Ocean.THydroMax_K)
                minTOcean_K = min(minTOcean_K, Planet.TfreezeLower_K if Planet.Do.ICEIh_THICKNESS else Planet.Bulk.Tb_K)
                deltaPOcean = min(deltaPOcean, Planet.Ocean.deltaP)
                deltaTOcean = min(deltaTOcean, Planet.Ocean.deltaT)
            if not Planet.Do.NO_ICE_CONVECTION:
    ...
    if GetIceEOSTracker:        
        ...
        for i, icePhase in enumerate(icePhases):
            if icePhase == 'Ih':
                ...
                GetPlanetIceEOS(Planet, Params, PHPIce_MPa, TOceanHPices_K, icePhase)
    ...
    if len(innerPlanets) > 0:
        ...
        for i, innerPlanet in enumerate(innerPlanets):
            GetPlanetInnerEOS(Planet, Params, Fe_EOS=False)
            if Planet.Do.Fe_CORE:
                GetPlanetInnerEOS(Planet, Params, Fe_EOS=True)
