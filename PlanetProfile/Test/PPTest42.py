"""
PPTest42
Maxwell ocean-bearing Titan: MCMC exploration of tidal dissipation parameters.

Configuration for the Maxwell rheology MCMC exploration on an ocean-bearing
Titan model. The interior structure varies with Tb_K (which controls ice shell
thickness and ocean depth), so the MCMC pre-computes structures at a grid of
Tb_K values and uses nearest-neighbor lookup during sampling.
...
Uses TidalPy backend with Maxwell rheology for all solid phases. The MCMC
runner (Test42_mcmc_maxwell_ocean.py) pre-computes structural arrays at
41 Tb_K values and calls TidalPy directly during sampling.

Physical context:
  With an ocean, the ice Ih shell responds nearly independently of the
  HP ice + silicate interior. The ocean layer (liquid, zero shear modulus)
  transmits only normal stress. Under Maxwell, HP ices with eta ~ 1e12-1e13
  are in the low-omega*tau regime (nearly Newtonian), so dissipation requires
  either very low ice viscosity or silicate dissipation near the Maxwell peak
  (eta ~ mu/omega ~ 5e14 Pa s).
...
""" Ice tidal heating — initial guess for ocean model """
Planet.Ocean.HtidalIce_Wm3 = 1e-8

""" Andrade rheology parameters (present but inert under Maxwell) """
