"""
PPTest38
TidalPy vs PyALMA3 backend comparison on Titan no-ocean model.
Identical to PPTest35 except uses TidalPy backend for gravity (Params.Gravity.backend = 'tidalpy').

Tests the TidalPy radial solver integration and validates:
  1. Love numbers agree with PyALMA3 to within ~2% after Andrade zeta conversion
  2. Per-layer tidal heating is computed (TidalPy-only feature)
  3. Energy conservation: sum of per-layer heating ~ E_dot from Im(k2) formula

Backend comparison results (PPTest35 model):
  PyALMA3: Re(k2)=0.574, Im(k2)=0.121, Q=4.7
  TidalPy: Re(k2)=0.578, Im(k2)=0.123, Q=4.7

Andrade zeta convention:
  PlanetProfile/PyALMA3 uses zeta as a linear inverse amplitude on Andrade creep:
    J_A = [Gamma(alpha+1) / zeta_pa] * (1/mu) * (s*tau_M)^(-alpha)
  TidalPy uses zeta as a timescale ratio inside the power law:
    J_A = Gamma(alpha+1) * (1/mu) * (omega*tau_M*zeta_tp)^(-alpha)
  Conversion: zeta_tidalpy = zeta_pyalma^(1/alpha)
  At zeta=1 both are identical.  For zeta_pa=0.005, alpha=0.2:
    zeta_tp = 0.005^5 = 3.125e-12 (applied automatically in _map_rheology_to_tidalpy)
...
Planet = PlanetStruct('Test38')

# Titan analog: same as PPTest35
...
""" Ice tidal heating — converged value """
Planet.Ocean.HtidalIce_Wm3 = 1.15e-7

""" Andrade rheology parameters (PyALMA3 convention zeta values) """
