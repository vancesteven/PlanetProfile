"""
PPTest44
Maxwell ocean-bearing Titan: MCMC with Arrhenius melting-point viscosity.

Same interior configuration as PPTest42 (Maxwell, ocean, Kalousova
convection, self-consistent tidal heating). The MCMC runner
(Test44_mcmc_maxwell_arrhenius_ocean.py) samples etaMelt — viscosity
at the melting temperature — and applies Arrhenius temperature scaling
at each radial point, rather than using flat per-phase viscosity overrides.

MCMC parameter space (4D):
  log10(etaMelt_Ih) (Pa s):    [10, 16]
  log10(etaMelt_HP) (Pa s):    [10, 16]
  log10(etaMelt_sil) (Pa s):   [12, 22]  (flat, no Arrhenius)
  Tb_K (basal temperature):    [251.2, 275.0]

Arrhenius formula:
  eta(T) = etaMelt * exp(Eact/R * (1/T - 1/Tmelt))
  with per-phase Eact and Tmelt from Constants.
...
Planet.Bulk.Tb_K = 265.0  # Default midpoint; MCMC varies 251.2-275
...
""" Melt-bearing ice viscosity (defaults; MCMC overrides etaMelt per-phase) """
