"""
MCMC exploration: Andrade rheology + Yao 2014 spherical convection, 2km Clathrate Titan (PPTest49_2km).

Perturbation check of Test48 (Path B):
1. Uses PPTest49_2km (2 km clathrate cap instead of 5 km).
...
log = logging.getLogger('PPTest49_2km_MCMC_Clath2km')
...
GRID_CACHE_PATH = os.path.join(OUTPUT_DIR, 'titan_yao2014_clathrate2km_hybrid_hydro_grid_cache.pkl')
    ...
    if os.path.exists(GRID_CACHE_PATH) and not force_rebuild:
        ...
        if n_pts < n_expected and not build_complete:
            ...
            cache_data = build_hybrid_hydrosphere_grid(
                'PlanetProfile.Test.PPTest49_2km',
                tb_grid,
                d_grid,
                GRID_CACHE_PATH,
                rheology='maxwell',
        ...
        log.info(f'Building grid for PPTest49_2km (2km Clathrate, {n_expected} points)...')
        cache_data = build_hybrid_hydrosphere_grid(
            'PlanetProfile.Test.PPTest49_2km',
            tb_grid,
            d_grid,
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='PPTest49_2km Andrade + Yao2014 2km-Clathrate MCMC')
    ...
    if not args.no_plots:
        ...
        out = lambda name: os.path.join(OUTPUT_DIR, f'titan_test49_clathrate2km_{name}.png')
        title = 'Titan (Yao 2014, 2km Clathrate)'
