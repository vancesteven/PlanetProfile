...
from PlanetProfile.Thermodynamics.ConstantEOS import GetConstantEOS, returnVal
    ...
    if DO_POROUS:
        ...
        if Planet.Do.CustomEOS['Ocean']:
            ...
            oceanEOSWrapper = GetOceanEOS(Planet.Ocean.comp, Planet.Ocean.wOcean_ppt, P_MPa, T_K,
                                        ...
                                        kThermConst_WmK=Planet.Ocean.kThermWater_WmK,
                                        propsStepReductionFactor=Planet.Ocean.propsStepReductionFactor)
...
def GetOceanEOS(compstr, wOcean_ppt, P_MPa, T_K, elecType, rhoType=None, scalingType=None, phaseType=None,
                ...
                etaFixed_Pas=None, kThermConst_WmK=None, propsStepReductionFactor=1):
    oceanEOS = OceanEOSStruct(compstr, wOcean_ppt, P_MPa, T_K, elecType, rhoType=rhoType, scalingType=scalingType,
                              ...
                              propsStepReductionFactor=propsStepReductionFactor)
...
class OceanEOSStruct:
    def __init__(self, compstr, wOcean_ppt, P_MPa, T_K, elecType, rhoType=None, scalingType=None,
                 ...
                 sigmaFixed_Sm=None, LOOKUP_HIRES=False, etaFixed_Pas=None, kThermConst_WmK=None, propsStepReductionFactor=1):
    ...
    def fn_Seismic(self, P_MPa, T_K, grid=False):
        if not self.EXTRAP:
            P_MPa, T_K = ResetNearestExtrap(P_MPa, T_K, self.Pmin, self.Pmax, self.Tmin, self.Tmax)
        # ufn_seismic returns Vp and Ks, but we must return zeros for Vs and Gs to keep compatibility with other seismic functions of other EOS
        VP, KS = self.ufn_Seismic(P_MPa, T_K, grid=grid)
        return VP, np.zeros_like(VP), KS, np.zeros_like(KS)
...
def GetPlanetIceEOS(Planet, Params, P_MPa, T_K, phaseStr, useMinRes = True):
    if Planet.Do.ConstantProps['Ice']:
        iceEOSWrapper = GetConstantEOS(Planet.Ocean.IceConstantProps.get(phaseStr), EOStype='ice', EOScomp=phaseStr)
    else:
        if Planet.Do.POROUS_ICE:
            iceEOSWrapper = GetIceEOS(P_MPa, T_K, phaseStr,
                            porosType=Planet.Ocean.porosType[phaseStr],
                            phiTop_frac=Planet.Ocean.phiMax_frac[phaseStr],
                            Pclosure_MPa=Planet.Ocean.Pclosure_MPa[phaseStr],
...
def GetIceEOS(P_MPa, T_K, phaseStr, porosType=None, phiTop_frac=0, Pclosure_MPa=0, phiMin_frac=0,
              ...
              ICEIh_DIFFERENT=False, etaFixed_Pas=None, TviscTrans_K=None, mixParameters=None, kThermConst_WmK=None):
        ...
        if not self.ALREADY_LOADED:
            ...
            if phaseStr == 'Clath':
                ...
                if (T_K[0] >= T_K[-1]) or (P_MPa[0] >= P_MPa[-1]):
                    ...
                    VP_kms = iceOut.Vp * 1e-3
                    VS_kms = iceOut.Vs * 1e-3
                    KS_GPa = iceOut.Ks * 1e-3
                    GS_GPa = iceOut.shear * 1e-3
            ...
            # Interpolate seismic fucntions and feed into seismic wrapper function to be compatible with other seismic functions of other layer types
            self.ufn_Vp_kms = RectBivariateSpline(P_MPa, T_K, VP_kms)
            self.ufn_VS_kms = RectBivariateSpline(P_MPa, T_K, VS_kms)
            self.ufn_KS_GPa = RectBivariateSpline(P_MPa, T_K, KS_GPa)
            self.ufn_GS_GPa = RectBivariateSpline(P_MPa, T_K, GS_GPa)
            self.ufn_Seismic = IceSeismic(self.ufn_Vp_kms, self.ufn_VS_kms, self.ufn_KS_GPa, self.ufn_GS_GPa)
...
class SFphase:
    ...
    def __call__(self, P_MPa, T_K, grid=False):
        if self.comp == 'water1':
            ...
            return WhichPhase(PT).astype(np.int8)
        else:
            if grid:
                PTm = sfPTmGrid(P_MPa, T_K, self.m_molal)
            else:
                PTm = self.PTmTrips(P_MPa, T_K)
            return WhichPhase(PTm, solute=self.comp).astype(np.int8)
...
class IceSeismic:
    def __init__(self, ufn_Vp_kms, ufn_VS_kms, ufn_KS_GPa, ufn_GS_GPa):
        self.ufn_Vp_kms = ufn_Vp_kms
        self.ufn_VS_kms = ufn_VS_kms
        self.ufn_KS_GPa = ufn_KS_GPa
        self.ufn_GS_GPa = ufn_GS_GPa

    def __call__(self, P_MPa, T_K, grid=False):
        return self.ufn_Vp_kms(P_MPa, T_K, grid=grid), self.ufn_VS_kms(P_MPa, T_K, grid=grid), self.ufn_KS_GPa(P_MPa, T_K, grid=grid), self.ufn_GS_GPa(P_MPa, T_K, grid=grid)
