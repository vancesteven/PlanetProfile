...
def SelfConsistentOceanLayer(Planet, Params):
    ...
    if Planet.Do.VALID and (not Planet.Do.NO_OCEAN or Planet.Do.NO_OCEAN_EXCEPT_INNER_ICES):
        ...
        if Planet.Do.NO_OCEAN_EXCEPT_INNER_ICES:
            ...
            thisPhase = Planet.Ocean.EOS.fn_phase(initialPOcean_MPa, TOcean_K[0]).astype(np.intp)
            ...
            if thisPhase < 2:
                ...
                thisPhase = Planet.Ocean.EOS.fn_phase(initialPOcean_MPa+Planet.Ocean.EOS.deltaP, TOcean_K[0]).astype(np.intp)
            ...
            if Planet.phase[Planet.Steps.nSurfIce] == 0 and alphaOcean_pK[0] < 0 and not Planet.Do.NO_MELOSH_LAYER:
                ...
                while alphaMelosh_pK < 0 and Planet.phase[Planet.Steps.nSurfIce+i] == 0:
                    ...
                    if alphaMelosh_pK > 0 or deltaPtop >= (i+1)*Planet.Ocean.deltaP:
                        ...
                        Planet.phase[Planet.Steps.nSurfIce+i] = Planet.Ocean.EOS.fn_phase(POcean_MPa[i], TOcean_K[i]).astype(np.intp)
...
def SelfConsistentInnerLayer(Planet, Params):
    ...
    if Planet.Do.VALID:
        ...
        Planet.phase = np.concatenate((Planet.phase[:Planet.Steps.nHydro], extend.astype(np.intp)))
...
def CalcMoIConstantRho(Planet, Params):
    ...
    if not Params.SKIP_INNER:
        ...
        phasePore = np.zeros((1, Planet.Steps.nSil), dtype=np.intp)
...
def CalcMoIWithEOS(Planet, Params):
    ...
    if Planet.Do.Fe_CORE:
        ...
        iValid = np.array([Planet.Steps.iSilStart + i for i in indsSilValid]).astype(np.intp)
        ...
        nSilFinal = Planet.Steps.nSilMax * np.ones_like(indsSilValid).astype(np.intp)
        Ccore_kgm2 = 0
    ...
    if len(CMR2inds) == 0:
        ...
        phaseSilPore = np.zeros((1, Planet.Steps.nSil+1), dtype=np.intp)
        coreProps = (np.zeros(Planet.Steps.nCore) for _ in range(9))
        Planet.Sil.phiCalc_frac = np.nan
