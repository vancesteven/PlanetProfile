...
def SilRecursionSolid(Planet, Params,
    ...
    # (no Fe core, no Do.ConstantProps['Inner']), the only physically admissible solution
    ...
    # a prescribed one.
    #
    # For SHELL bodies (Fe_CORE=True or Do.ConstantProps['Inner']=True) the c1/r term is
    ...
    isSolidSphere = (not Planet.Do.Fe_CORE) and (not Planet.Do.ConstantProps['Inner'])
...
def SilRecursionPorous(Planet, Params,
    ...
    # Do.ConstantProps['Inner']) retain the legacy halved-c1 ConductiveTemperature call.
    isSolidSphere = (not Planet.Do.Fe_CORE) and (not Planet.Do.ConstantProps['Inner'])
