...
def PlotInductOgramPhaseSpace(results_list, FigureFilesList, Params):
    ...
    # Get axis labels and other properties
    ...
    if Params.COMPARE and np.size(results_list) > 1 and Params.Induct.inductOtype != 'sigma':
        ...
        comboX_data = np.concatenate(tuple(xi for xi in x_data))
        ...
        for comp in oceanComp:
            ...
            # Only add bounds for supported inductOtypes
            if FigMisc.MARK_INDUCT_BOUNDS and Params.Induct.inductOtype != 'oceanComp':
                thisX_data = comboX_data[thisComp]
        ...
        plt.tight_layout()
        fig.savefig(Params.FigureFiles.phaseSpaceCombo, format=FigMisc.figFormat, dpi=FigMisc.dpi, metadata=FigLbl.meta, transparent=FigMisc.TRANSPARENT)
...
def PlotInductOgram(results_list, FigureFilesList, Params):
    for resultListIndex, result in enumerate(results_list):
        ...
        if Params.COMBINE_BCOMPS:
            ...
            [ax.set_xlabel(x_label) for ax in (axes[1,0], axes[1,1])]
            ...
            for z, name, fLabel in zip(comboData, comboTitles, comboLabels):
                ...
                ax.set_title(name)
                zContours = [ax.contour(contour_x, contour_y, z[i, ...],
                ...
                zLabels = [plt.clabel(zContour, fmt=Params.Induct.GetCfmt(fLabel, T), inline=True)
        ...
        for z, name, fLabel in zip(zData, FigLbl.plotTitles, FigLbl.fLabels):
            ...
            zContours = [axes[0].contour(contour_x, contour_y, z[i, ...],
                            colors=Color.Induction[T], linestyles=Style.LS_Induction[T],
                            linewidths=Style.LW_Induction[T], levels=Params.Induct.GetClevels(fLabel, T))
                            for i,T in enumerate(TexcPlotNames[iSort])]
            ...
            zLabels = [axes[0].clabel(zContour, fmt=Params.Induct.GetCfmt(fLabel, T), inline=True)
                        for zContour, T in zip(zContours, TexcPlotNames[iSort])]
            phaseLabels = [axes[1].clabel(phaseContour, fmt=Params.Induct.GetCfmt('phase', T), inline=True)
            ...
            plt.tight_layout()
            fig.savefig(Params.FigureFiles.induct[fLabel], format=FigMisc.figFormat, dpi=FigMisc.dpi, metadata=FigLbl.meta, transparent=FigMisc.TRANSPARENT)
...
def PlotComplexBdip(PlanetList, Params):
    if PlanetList[0].bodyname in Excitations.Texc_hr.keys():
        refs = {}
        nPeaksToPlot, peaksToPlotNames = countPlottableExcitations(PlanetList[0].Magnetic.calcedExc, Params.Induct)
        if nPeaksToPlot >= 1:
            if nPeaksToPlot == 1:
                DO_ZOOM = False
                nCol = 1
            ...
            if Params.COMBINE_BCOMPS:
                ...
                plt.tight_layout()
                fig.savefig(Params.FigureFiles.Bdip['all'], format=FigMisc.figFormat, dpi=FigMisc.dpi, metadata=FigLbl.meta, transparent=FigMisc.TRANSPARENT)
                ...
                for axComp in xyzComps:
                    ...
                    plt.tight_layout()
                    fig.savefig(Params.FigureFiles.Bdip[axComp], format=FigMisc.figFormat, dpi=FigMisc.dpi, metadata=FigLbl.meta, transparent=FigMisc.TRANSPARENT)
            ...
            for iEval, tEval_s in enumerate(FigMisc.tMagEval_s):
                ...
                if np.size(PlanetListSubset) == 1:
                    ...
                    if not (Params.CALC_ASYM and FigMisc.BASYM_WITH_SYM):
                        ...
                        fig.savefig(fName, bbox_inches='tight', format=FigMisc.figFormat, dpi=FigMisc.dpi, metadata=FigLbl.meta, transparent=FigMisc.TRANSPARENT)
...
def PlotMagSpectrumInduced(PlanetList, Params):
    ...
    plt.tight_layout()
    fig.savefig(Params.FigureFiles.MagFT, format=FigMisc.figFormat, dpi=FigMisc.dpi, metadata=FigLbl.meta, transparent=FigMisc.TRANSPARENT)
...
def PlotMagSpectrum(PlanetList, Params):
    ...
    plt.tight_layout()
    fig.savefig(Params.FigureFiles.MagFTexc, format=FigMisc.figFormat, dpi=FigMisc.dpi, metadata=FigLbl.meta, transparent=FigMisc.TRANSPARENT)
