...
| titan_freegrav_nh3_posterior_1m.pt | test54_titan_nh3_freegrav.json | e596574d1e81567c (sbi; mcmc-mode 1611b65f, mode-only diff) | train 72 / data 72 / noise 7272 | 689,845 (nsf, k2-support box + drop_nonfinite, 31.0% rejected) | **SBC PASS** (353 pairs, min BH-adj KS p .772, all raw KS p .24–.94); crosscheck FAIL only on 3 unconstrained eta medians (eta_Ih/III/VI ≤.28σ; all load-bearing params pass mean/sigma/median/shape); limits FAIL = monotonicity DIRECTION on eta_Ih, N/A for a joint mixture (all in-support, containment/interior pass) | **RATIFIED (verified) — scientific-reviewer 2026-08-03.** Titan free-gravity JOINT no-ocean+ocean NH3 (obs {C20,C22,Re_k2,Im_k2}; 13 params incl. log10_wOcean_ppt over 1–70 ppt). Frozen (Tb,w) nodes build as REAL no-ocean interiors and appear in the posterior (user decision 2026-07-30; `retry_frozen_as_no_ocean=True` cache sha256 3d837cf8…). Caveats: eta_Ih/III/VI are prior-dominated nuisances (don't quote SBI point estimates); limits monotonicity gate N/A for joint mixtures; ~1.1–1.3× SBI/pocoMC sigma (CIs marginally conservative, SBC supersedes). Full verdict: `validation_reports/titan_freegrav_nh3_1m/RATIFICATION.md`. GUI slot wired (replaced placeholder); in-browser render pending. |
