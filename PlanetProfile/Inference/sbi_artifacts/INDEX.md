...
Artifact ratification uses the raw validation-gate results plus the recorded
scientific adjudication; it is not an "all three gates must PASS" rule. A GUI
slot may deploy when the applicable gates PASS, or when each surviving FAIL has
a committed adjudication and the slot carries the required scope note, domain
guard, or sector warning. An acceptable adjudicated failure is recorded as
`FAIL-ADJUDICATED-ACCEPTABLE`, never relabeled PASS (vocabulary ruling:
`plans/active/europa-v5v6v7-gate-adjudication.md`). Split ratification must name
the trusted deliverable and quarantine the unverified sector, with its
authoritative fallback, in the visible slot warning.

The standard raw gates are (`PlanetProfile/Inference/validate_sbi.py`;
thresholds in `plans/monte-carlo-sbi-implementation-plan.md` §Validation):
...
|---|---|---|---|---|---|---|---|
| titan_andrade_noocean_posterior.pt | test50_titan_noocean_andrade_8D.json | 629afbd55a4f0ce5 | bf7c938e | train 42 / data 44 / noise 4444 | 999,816 (nsf) | ALL GREEN within domain (amended rules, 2026-07-11). Machine A cross-version gate: `PlanetProfile/Inference/sbi_artifacts/validation_reports/cross_version_machineA/titan_crosscheck_torch28.json`. | 2026-07-12 (user-ratified; artifact pushed git 4d32809b, pre-deploy assertions green) |
| titan_freegrav_nh3_posterior_1m.pt | test54_titan_nh3_freegrav.json | e596574d1e81567c (SBI); 1611b65f (MCMC, mode-only diff) | 33a7ef9e | train 72 / data 72 / noise 7272 | 689,845 (nsf, k2-support box + drop_nonfinite, 31.0% rejected) | **SBC PASS** (353 pairs, min BH-adj KS p .772, all raw KS p .24–.94); **crosscheck FAIL** — three eta medians breach the marginal gate, and the posterior-predictive check exposes a joint tidal-sector under-update that the per-parameter gate cannot see; do not read this as benign nuisances only. Limits FAIL is mis-specified/N/A for the joint mixture (all in-support; containment/interior pass). | **SPLIT ratification — scientific-reviewer 2026-08-03.** VERIFIED for the gravity/structure sector (C20, C22, R_core, rho_core, salinity, Tb, and null non-hydrostatic offsets); NOT VERIFIED for the tidal k2/dissipation sector (Re_k2, Im_k2, log10_zeta, eta viscosities), for which the MCMC reference is authoritative. Frozen (Tb,w) nodes build as real no-ocean interiors and remain in the posterior. Full verdict: `validation_reports/titan_freegrav_nh3_1m/RATIFICATION.md`. GUI slot wired; in-browser render pending. |
| titan_freegrav_mgso4_posterior_1m.pt | test54_titan_mgso4_freegrav.json | d1bfdf1ee55bc737 (SBI); d0807949c01e1a81 (MCMC) | b5c4bec8 | train 73 / data 73 / noise 7373 | 691,075 kept/1M (30.9% rejected) | **SBC PASS 13/13** (n_sbc=1500, BH-FDR, zero fails); **crosscheck PASS 13/13** vs pooled n_eff~2000 B3 reference; **limits**: containment PASS (max shift 0.004 sigma), monotone clause gated FAIL overridden by adjudicator (falsified-premise, obs Im_k2 0.135 < 0.15 boundary) — recorded, not relabeled; **pushforward**: gravity centers on obs (<=0.05 sigma signed), Re_k2 reproduces model-data tension (pp median 0.570 vs 0.608; flow-vs-MCMC median-to-median gap 0.24 sigma, within 0.5 sigma rule), Im_k2 under-predicts (-1.60 sigma) -> tidal quarantine STANDS. Reports: `validation_reports/titan_freegrav_mgso4_1m/`. | **SPLIT ratification (per-composition) 2026-08-10, manager-countersigned** after independent verification of pushforward/SBC/crosscheck numbers from the committed reports. Gravity/structure TRUSTED; Re_k2 informative-with-caveat (deployed SBI marginal CONSERVATIVE vs reference MCMC); Im_k2/zeta/eta QUARANTINED (MCMC authoritative). GUI slot wired 2026-08-10 with sector warning carrying the reviewer-mandated caveat copy; AppTest verified. |
| titan_freegrav_nacl_posterior_1m.pt | test54_titan_nacl_freegrav.json | 5b3d2502546548c3 (SBI); de02aae94934f51b (MCMC) | b5c4bec8 | train 74 / data 74 / noise 7474 | 631,214 kept/1M (36.9% rejected) | **SBC PASS 13/13** (n_sbc=1500, BH-FDR, zero fails); **crosscheck 12/13** vs pooled B3 reference — `log10_eta_V = FAIL-ADJUDICATED-ACCEPTABLE` (median 0.352 vs 0.30 dex tol; poorly-identified HP-ice-V nuisance, threshold-adjacent to MgSO4's passing 0.209; all observable-relevant params PASS); **limits**: containment PASS (max shift 0.007 sigma), monotone clause gated FAIL overridden (falsified-premise); **pushforward**: gravity centers on obs (<=0.05 sigma), Re_k2 median-to-median gap 0.53 sigma adjudicated a mechanical threshold artifact (same tension direction as MCMC), Im_k2 under-predicts (-1.34 sigma) -> tidal quarantine STANDS. Reports: `validation_reports/titan_freegrav_nacl_1m/`. | **SPLIT ratification (per-composition) 2026-08-10, manager-countersigned** after independent verification (incl. reproducing the eta_V FAIL from the committed crosscheck report). Gravity/structure TRUSTED; Re_k2 informative-with-caveat (CONSERVATIVE vs reference MCMC); Im_k2/zeta/eta QUARANTINED + eta(ice V) nuisance marginal do-not-cite. GUI slot wired 2026-08-10 with sector warning; AppTest verified. |
...
Candidate, diagnostic, and subsequently deployed artifacts whose evidence was
first committed for cross-machine verification. The three wired Titan joint
composition rows are cataloged in the deployed table above.
...
| europa_clipper_v5_geodesy_11D_posterior_1m.pt | europa_clipper_v5_geodesy_11D.json | 0781749945edc44b | train 51 / data 50 / noise 5050 | 381,734 | 2026-08-06 gates vs FRESH pooled n_eff~2000 reference (n_sbc=1500, 621 kept): SBC 10/11 PASS, dC22_nh FAIL-ADJUDICATED-ACCEPTABLE (raw p=0.0017, BH-adj 0.0185; localized v5-flow undertraining of ~100:1 nuisance; v6 identical nuisance PASSES p=0.61); crosscheck PASS (D_iceIh d_excess 0.0171<=0.0281 even without the preregistered 0.36 km floor); limits PASS; B6 anchor N/A-by-domain. See `validation_reports/v5_gate_summary.json`. | **RATIFIED (scoped) 2026-08-06** — ships for the D_iceIh/ocean/salinity deliverable WITH mandatory scope note: dC22_nh marginal + non-hydrostaticity readout deferred to v6. 2nd-seed SBC queued non-blocking. plans/active/europa-v5v6v7-gate-adjudication.md (Final ratification 2026-08-06). GUI gating lifted; wired for deployment. |
| europa_clipper_v5_noinduction_7obs_posterior_1m.pt | europa_clipper_v5_noinduction_7obs.json | bbf023a694aa95b0 | train 51 / data 50 / noise 5050 | 381,734 | 2026-08-06 arm gates vs fresh pooled reference: see `validation_reports/v5_gate_summary.json` (per-arm dir europa_clipper_v5_noinduction_1m). | **RATIFIED (scoped) 2026-08-06** as ablation companion to the v5 baseline (same scope note). plans/active/europa-v5v6v7-gate-adjudication.md (Final ratification 2026-08-06). GUI gating lifted. |
| europa_clipper_v5_nok2_17obs_posterior_1m.pt | europa_clipper_v5_nok2_17obs.json | 077875a481f337b0 | train 51 / data 50 / noise 5050 | 381,734 | 2026-08-06 arm gates vs fresh pooled reference: see `validation_reports/v5_gate_summary.json` (per-arm dir europa_clipper_v5_nok2_1m). | **RATIFIED (scoped) 2026-08-06** as ablation companion to the v5 baseline (same scope note). plans/active/europa-v5v6v7-gate-adjudication.md (Final ratification 2026-08-06). GUI gating lifted. |
| europa_clipper_v6_freegrav_11D_posterior_1m.pt | europa_clipper_v6_freegrav_11D.json | a8cd890fff6a0023 | train 61 / data 67 / noise 6767 | 381,093 | 2026-08-06 gates (n_sbc=1500, 560 kept): SBC 11/11 PASS (dC22_nh BH-adj p=0.85); crosscheck PASS (stock HEAD, no floor); limits PASS; B6 anchor N/A-by-domain. See `validation_reports/v6_gate_summary.json`. | **RATIFIED 2026-08-06; CANONICAL Clipper–Europa model (user-ratified 2026-08-11)** — all gates PASS; owns the dC22_nh/non-hydrostaticity deliverable. plans/active/europa-v5v6v7-gate-adjudication.md (Final ratification 2026-08-06). GUI gating lifted; wired for deployment. |
| europa_clipper_v6_freegrav_noinduction_6obs_posterior_1m.pt | europa_clipper_v6_freegrav_noinduction_6obs.json | 7075e2ae5dca104b | train 61 / data 67 / noise 6767 | 381,093 | 2026-08-06 arm gates: see `validation_reports/v6_gate_summary.json` (per-arm dir europa_clipper_v6_noinduction_1m). | **RATIFIED 2026-08-06** as ablation companion to the v6 baseline. plans/active/europa-v5v6v7-gate-adjudication.md (Final ratification 2026-08-06). GUI gating lifted. |
| europa_clipper_v6_freegrav_nok2_16obs_posterior_1m.pt | europa_clipper_v6_freegrav_nok2_16obs.json | 70b8e622dd28c807 | train 61 / data 67 / noise 6767 | 381,093 | 2026-08-06 arm gates: see `validation_reports/v6_gate_summary.json` (per-arm dir europa_clipper_v6_nok2_1m). | **RATIFIED 2026-08-06** as ablation companion to the v6 baseline. plans/active/europa-v5v6v7-gate-adjudication.md (Final ratification 2026-08-06). GUI gating lifted. |
| europa_clipper_v7_openae_11D_posterior_1m.pt | europa_clipper_v7_openae_11D.json | 6b77483322378820 | train/data/gates 71 | 955,816 | SBC PASS; limits FAIL; crosscheck FAIL. See `validation_reports/v7_gate_summary.json` and `validation_reports/europa_clipper_v7_openae_1m/`. | **RETIRED as a deployment candidate (user-ratified 2026-08-11)** — the crosscheck blocker dissolved with the B3 fresh-reference work (reference-side artifact); retained as a published \|Ae\| support-cut sensitivity ablation using the B3 references. B4 evidence check still runs; B5 cancelled. Never wired. plans/active/europa-v5v6v7-gate-adjudication.md. |
| titan_freegrav_noocean_posterior_1m.pt | titan_freegrav_noocean.json | 9600b13e519baea6 | train/data/gates 71 | 877,227 | SBC PASS; limits FAIL; crosscheck FAIL. See `validation_reports/titan_freegrav_noocean_1m/titanG_gate_summary.json`. | **DEPLOYED 2026-07-24.** GUI slot `titan_phase_a_noocean` is live at `PlanetProfileApp/pages/Inference.py`; reviewer VERIFIED the interior-agnostic gravity deliverable. The crosscheck and limits exits are FAIL-adjudicated-acceptable under the slot's documented scope note. |
