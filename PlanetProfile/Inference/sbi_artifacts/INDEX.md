...
| europa_galileo_v1p1_8D_posterior_1m.pt | europa_galileo_v1p1_8D.json | (see artifact) | train 44 / data 50 / noise 5050 | 1m-class (nsf, synodic \|Ae\|>0.7 support cut) | **ALL GATES PASS**: SBC 8/8 (min KS p .067), crosscheck 8/8 (no soft fails); zeta split decouples ice/silicate heating (corr -0.02, MCMC & SBI identical); runner-path heating reproduces stored reference exactly | **DEPLOYED 2026-07-20** (slot swap for v1 per the retirement schedule). Honest-data framing: only CMR2 (GC21) + synodic support cut are Galileo-era; k2/h2 are labeled hypothetical channels (Re_k2 [0.23, 0.05], Im_k2 [0.004, 0.05]). Machine A end-to-end GUI sampling `verified`: 10k draws; all 8 marginal means match the stored crosscheck MCMC stats within 0.5 sigma_MCMC. Guards: Im_k2 <= 0.15 (conservative carryover; no v1.1 anchor walk), Tb >= 261.5 K default truncation. Gates: validation_reports/europa_galileo_v1p1_1m/PHASE_GATES.md. |
| europa_clipper_v4_geodesy_11D_posterior_1m.pt | europa_clipper_v4_geodesy_11D.json | (see artifact) | train 43 / data 49 / noise 4949 | 326,849 kept (nsf, 67.3% support rejection, 21-D x / 11-D theta) | **DELIVERABLE VERIFIED**: Gate 4 non-hydrostatic u = dC22_nh + dC20_nh/3.324 recovered 94-95%, likelihood-dominated (36x prior shrink), u-SBC calibrated (KS p .093, conservative); Tb-w joint PASS (corr -.987 vs ref -.983, containment .640/.920); flow-fidelity PASS (PPD KS p .121). SBC/crosscheck/degeneracy FAIL only on prior-dominated interior scalars (zeta_Ih, rho_core) + diffuse Tb - diagnosed intrinsic (robustk2 ablation ruled out preprocessing; Re_k2 0.23 sits a genuine, MCMC-reproduced 1.3 sigma below the joint PPD). Reviewer: PASS, close v4, no blocker | **USER-RATIFIED + DEPLOYED 2026-07-20** (user authorized HF ship). sampling `verified`. GUI slot live (config_path, inherited Im_k2 <= 0.15 + synodic \|Ae\| [0.75,0.94] guards, no Tb truncation - 2D support); Machine A end-to-end GUI sampling: 10k draws, corr(Tb, log10w) -0.986 vs committed reference -0.982, u sigma 3.35e-7 (= gate report), per-sample c20/c22 packaged, new u-panel renders (reportable upper limit; per-component marginals marked do-not-cite). Reference MCMC: Test/mcmc_results/Europa/Test53_geodesy_v4/. Gates: validation_reports/europa_clipper_v4_1m/PHASE_GATES.md. |
| europa_clipper_v5_geodesy_11D_posterior_1m.pt (+ noinduction_7obs, nok2_17obs siblings) | europa_clipper_v5_geodesy_11D.json (+ 2 ablation configs) | (see artifact) | data 50 / noise 5050 / train 51 | 1m-class (nsf, shared 2D Tb×w cache; baseline 11D theta / 21-obs x) | Trio column-sliced from ONE 1M dataset. Gate summary `validation_reports/v5_gate_summary.json` (seed 51): baseline sbc 0 (PASS) / limits 2 / crosscheck 2; noinduction sbc 0 / limits 2 / crosscheck n/a (no ref MCMC for ablation); nok2 sbc 2 / limits skipped (no k2) / crosscheck n/a. Ablation shows induction carries the D_iceIh↔log w correlation (baseline corr .923, noinduction .539). Reference MCMC: Test/mcmc_results/Europa/Test52_seawater_v5/. | **DEPLOYED** (GUI slot trio live; channel-family selector all/no_induction/no_k2h2). v5 = v4 geodesy + D_iceIh ice-thickness reparam. Guards inherited from v4 (Im_k2 ≤ 0.15, synodic \|Ae\| [0.75,0.94]). Per-arm gate reports: validation_reports/europa_clipper_v5_{baseline,noinduction,nok2}_1m/ + phase0. |
| europa_clipper_v6_freegrav_11D_posterior_1m.pt (+ noinduction_6obs, nok2_16obs siblings) | europa_clipper_v6_freegrav_11D.json (+ 2 ablation configs) | (see artifact) | data 67 / noise 6767 / train 61 | n_train 381,093 (nsf, shared 2D Tb×w cache; baseline 11D theta / 20-obs x) | **CMR2 DROPPED as observable** (0.3547 is a hydrostatic Radau reduction of C22 — conditioning on it double-counts C22 and biases toward hydrostaticity). C20/C22 = GC21 Table 2 SOL-A unconstrained/unnormalized (C20 -4.3759e-4±7.747e-5, C22 1.3862e-4±2.44e-6); widened agnostic offset boxes (dC20_nh ±3.9e-4, dC22_nh ±5e-5) → gravity carries ~zero interior C/MR² info; interior constrained by k2/h2 + induction. Gate summary `validation_reports/v6_gate_summary.json` (seed 61): baseline sbc 0 (PASS) / limits 2 / crosscheck 2; noinduction sbc 2 / limits 2 / crosscheck n/a; nok2 sbc 0 / limits skipped / crosscheck n/a. All non-PASS codes are benign design consequences (OOD limit sweep; fine-shape crosscheck on weakly-identified params; multiplicity-expected marginal SBC). Reference MCMC: Test/mcmc_results/Europa/Test53_seawater_v6/. | **DEPLOYED 2026-07-23** (GUI slot trio live; channel-family selector all/no_induction/no_k2h2; render + posterior `verified` via AppTest + panel-PDF inspection, hydro-reference median 0.35551 = +0.3σ from GC21 matching design). scientific-reviewer **PASS WITH CONCERNS 2026-07-23** (gate "failures" benign). C/MR² panel gains a **published-C/MR² display overlay** (GC21 default; Anderson 1998, Jacobson et al. 1999 selectable) — DISPLAY REFERENCE ONLY, no reweighting, no interior-C/MR² claim until the Task D reweighter (plan #36). Guards inherited: Im_k2 ≤ 0.15 (baseline), synodic \|Ae\| [0.75,0.94]. Per-arm gate reports: validation_reports/europa_clipper_v6_{baseline,noinduction,nok2}_1m/. |
